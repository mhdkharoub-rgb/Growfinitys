...
app.post('/payments/approve', async (req, res) => {
  ...
  try {
    ...
    // Step 1: Call Pi Network API to approve the payment
    // This is REQUIRED — Pi waits for this before showing confirm button to user
    ...
    try {
      ...
      console.log(`✅ Pi API approved payment:`, JSON.stringify(approveRes.data));
    } catch (piErr) {
      console.error(`⚠️ Pi API approve failed:`, piErr.response?.data || piErr.message);
      // Continue anyway — store locally
    }

    // Step 2: Store payment locally
...
app.post('/payments/complete', async (req, res) => {
  ...
  try {
    ...
    // Tell Pi Network we fulfilled the order (both sandbox and production)
    if (process.env.PI_API_KEY && txId) {
      try {
        const axios = require('axios');
        const completeRes = await axios.post(
          `https://api.minepi.com/v2/payments/${paymentId}/complete`,
          { txid: txId },
          { headers: { Authorization: `Key ${process.env.PI_API_KEY}` }, timeout: 10000 }
        );
        console.log('✅ Pi Network notified of completion:', JSON.stringify(completeRes.data));
      } catch(e) {
        console.error('Pi completion notify failed (non-fatal):', e.response?.data || e.message);
      }
    }
